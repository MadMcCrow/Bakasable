...
genericLinux = args : (generic args).overrideAttrs {
  nativeBuildInputs = [
...
genericDarwin = args : (generic args).overrideAttrs {
  nativeBuildInputs = [];
  installPhase = ''
    runHook preInstall
...
# TODO :
export = {version} : "${godotURL}/${version}/Godot_v${version}_export_templates.tpz";

# select binary
in if stdenv.isDarwin then genericDarwin {
